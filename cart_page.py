    ...
    def checkout():
        ...
        if total_cost:
            if not all([first_name, last_name, email, address, governorate, country, phone_code, phone_number, payment_method]):
                ...
                return
            # Insert a new order with total cost and quantity
            else:
                cursor.execute(
        ...
        # cursor.execute("""
        # INSERT INTO Orders (first_name, last_name, email, address, governorate, country, phone_code, phone_number, payment_method, discount_code)
        # VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
